...
def ImgRotate(img_input, coldepth, deg, direction):
    """
    Memutar gambar berdasarkan arah tertentu (searah atau berlawanan jarum jam).
    
    Args:
        img_input (PIL.Image): Objek gambar input yang akan diputar.
        coldepth (int): Kedalaman warna gambar (1, 8, atau 24/32).
        deg (int): Derajat rotasi (tidak digunakan langsung dalam implementasi ini, hanya 90 derajat).
        direction (str): Arah rotasi ('C' untuk clockwise/searah jarum jam, lainnya untuk counterclockwise).
    
    Returns:
        PIL.Image: Objek gambar output yang telah diputar.
    """
    # Konversi gambar ke mode RGB jika bukan 24-bit untuk memudahkan manipulasi
    if coldepth != 24:
        img_input = img_input.convert('RGB')
    
    # Buat gambar baru dengan ukuran terbalik (lebar jadi tinggi, tinggi jadi lebar) untuk rotasi 90 derajat
    ...
    # Iterasi setiap piksel untuk memetakan posisi baru berdasarkan arah rotasi
    for i in range(img_output.size[0]):  # Loop melalui lebar gambar output
        for j in range(img_output.size[1]):  # Loop melalui tinggi gambar output
            if direction == "C":  # Rotasi searah jarum jam (clockwise)
                r, g, b = img_input.getpixel((j, img_output.size[0] - i - 1))  # Ambil piksel dari posisi yang diputar
            else:  # Rotasi berlawanan jarum jam (counterclockwise)
                r, g, b = img_input.getpixel((img_input.size[1] - j - 1, i))  # Ambil piksel dari posisi yang diputar
            pixels[i, j] = (r, g, b)  # Simpan nilai RGB ke piksel output
    ...
    return img_output  # Kembalikan gambar hasil rotasi
